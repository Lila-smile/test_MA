...
class Env_battery:
    ...
    def step(self, action, num):
        ...
        next_state = np.array([HE_soc[-1][1], HP_soc[-1][1],self.Speed_vector[num][1], self.Power_total[num][1]])
        ...
        if next_state[0] <= 0.1 or next_state[1] <= 0.1:  # HE battery soc = 0 or HP battery soc = 0
            ...
            reward_function = - (HE_cur**2 *self.HE_battery.R0 + HE_v1**2/self.HE_battery.R1 + HE_v2**2/self.HE_battery.R2  + HP_cur**2 *self.HP_battery.R0+ HP_v1**2/self.HP_battery.R1 + HP_v2**2/self.HP_battery.R2 + 10*(HP_soc[-1][1]-0.5)**2)
